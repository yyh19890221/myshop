...
//  更新内容：
//       1.2.2a:增加touch事件机制目前调试中。
//       1.3.0a:修正了setinterval在移动端的webkit中只执行一次的bug
//		 1.3.0b:文本修改
/*
Demo:
$(".productshow").Xslider({
...
navEventType: "mouseenter"//nav触点的事件类型 默认为mouseenter
...
(function($) {
    ...
    var init = function(container, settings) {
        ...
        //SECTION 1 Start:auto save units viewsize 
        if (settings.unitLen == "auto") {
            settings.autoLen = true;
            settings.unitLen = false;
        } else {
            settings.autoLen = false;
        }
        //SECTION 1 End
        self.container.each(function() {
            ...
            var runenv = {
                ...
                unitLen: unitLen,
                autoLen: settings.autoLen,
                unitsView: [], //units height or width
            ...
            //SECTION 1 Start: environment variable initialization
            //save units viewsize
            //to be continued... 
            var allViewSize = 0;
            if (settings.autoLen) {
                if (settings.dir == "H") {
                    runenv.scrollunits.each(function(i, v) {
                        runenv.unitsView[i] = $(v).width();
                        allViewSize += $(v).outerWidth();
                    })
                } else if (settings.dir == "V") {
                    runenv.scrollunits.each(function(i, v) {
                        runenv.unitsView[i] = $(v).height();
                        allViewSize += $(v).outerHeight();
                    })
                }
                runenv.scrollObjSize = settings.scrollObjSize || allViewSize;
            } else {
                /*runenv.scrollunits.each(function (i, v) {
                runenv.unitsView[i] = settings.unitLen;
                allViewSize += settings.unitLen;
                });*/
            }
            ...
            if (settings.loop == "cycle") {
                ...
                for (var i = 0; i < runenv.unitDisplayed; i++) {
                    ...
                    beforeobj.attr("index", ~ ~beforeobj.attr("index") - runenv.unitDisplayed - 1); //逻辑错误
                    afterobj.attr("index", ~ ~afterobj.attr("index") + runenv.unitDisplayed + 1); //逻辑错误
    ...
    var ClassSlider = function(container, settings) {
        ...
        self.container.each(function() {
            ...
            //SECTION 2 TouchEvent
            /*var touchCtner = runenv.scrollObj.parent()[0];
            var startX, startY, x, y, touchoffsetX = 0;
            self.mouseflg = true;
            ...
            startX = touch.pageX;
            startY = touch.pageY;
            touchoffsetX = parseFloat(runenv.scrollObj.css("left"));
            self.mouseflg = false;
            }
            function touchMove(event) {
            event.preventDefault();
            if (self.mouseflg) return;
            var touch = event.targetTouches[0];
            x = touch.pageX - startX;
            y = touch.pageY - startY;
            runenv.scrollObj.css("left", touchoffsetX + x);
            }
            function touchEnd(event) {				
            event.preventDefault();
            if (self.mouseflg) return;
            self.mouseflg = true;
            if (runenv.unitLen / 2 <= Math.abs(x)) {
            var curindex = ~ ~me.data("curindex");
            var offsetgo = Math.round(x / runenv.unitLen);
            fnmove.call(me, me.data("curobj"), curindex - offsetgo, self.settings, false);
            } else {
            runenv.scrollObj.stop().animate({ "left": touchoffsetX }, 200);
            }
            }
            if (touchCtner && touchCtner.addEventListener) {
            touchCtner.addEventListener("touchstart", touchStart, false);
            touchCtner.addEventListener("touchmove", touchMove, false);
            touchCtner.addEventListener("touchend", touchEnd, false);
            touchCtner.addEventListener("touchcancel", touchEnd, false);
            }*/
            ////SECTION 2 End TouchEvent
        ...
        animate: function(obj, w, dir, unitLen, speed, callback, noAnimate) {
            if (dir == "H") {
                ...
                if (noAnimate) {
                    ...
                    var len = runenv.scrollunits.length;
                    $(runenv.scrollunits.selector, me).each(function(i, v) {
                        $(this).css("z-index", len - i);
                    })
